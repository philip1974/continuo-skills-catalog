...
1. `mkdir <plans_dir>/.alloc.lock` (acquire). Retry up to 60s. Stale check: if `started_at > 10 min ago`, `rmdir` and retry once. On final failure, abort.
2. Inside lock:
   ...
   - Write a **placeholder frontmatter** to `<topic_dir>/req.md`:
     ...
     ```
3. **Release lock**: `rmdir <plans_dir>/.alloc.lock`. Total lock hold time = single-digit seconds.
